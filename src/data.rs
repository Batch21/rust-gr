...
pub fn load_csv_data(path: PathBuf) -> Data {
...
pub fn load_parameter_data(path: PathBuf) -> Parameters {
    let file = fs::read_to_string(path).expect("Unable to read file");
    let params: Parameters = serde_json::from_str(&file).unwrap();
    params
}

pub fn save_flow(dates: Vec<String>, flows: Vec<f64>, output_file:PathBuf) -> Result<(), Box<dyn Error>> {
